...
**EU Commission Civic Tech Hackathon · Brussels · 22-23 June 2026**
...
cp civicconnect.env.example civicconnect.env
# Edit civicconnect.env and fill in your API keys
```

### 2. Secrets (required in `civicconnect.env`)

| Variable | Where to get it |
|---|---|
| `CIVICCONNECT_TELEGRAM_TOKEN` | [@BotFather](https://t.me/BotFather) on Telegram |
...
TOKEN=$(grep TELEGRAM_TOKEN civicconnect.env | cut -d= -f2 | tr -d '"')
...
User identities are stored as `SHA-256("civicconnect:{telegram_user_id}")` — preventing double-voting while making it impossible to trace a vote back to a specific person.

---

## Hackathon context

Built at the **EU Commission Civic Tech Hackathon** (Brussels, 22-23 June 2026) as part of the **LOOP Team**. CivicConnect was one of 3 projects highlighted by the jury out of all submissions.
